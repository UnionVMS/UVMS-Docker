...
-- ERS
CREATE USER ers WITH PASSWORD 'ers';
CREATE SCHEMA AUTHORIZATION ers;
ALTER USER ers SET search_path = ers, public;
GRANT CONNECT ON DATABASE db71u TO ers;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA ers TO ers;
ALTER SCHEMA ers OWNER TO ers;

-- USM for ERS
CREATE USER usm_ers WITH PASSWORD 'usm_ers';
CREATE SCHEMA AUTHORIZATION usm_ers;
ALTER USER usm_ers SET search_path = usm_ers, public;
GRANT CONNECT ON DATABASE db71u TO usm_ers;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA usm_ers TO usm_ers;
ALTER SCHEMA usm_ers OWNER TO usm_ers;
